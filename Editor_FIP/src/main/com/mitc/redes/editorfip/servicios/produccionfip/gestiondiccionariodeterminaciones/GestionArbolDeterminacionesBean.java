...
@Stateful
@Name("gestionArbolDeterminaciones")
public class GestionArbolDeterminacionesBean extends GestionArbolGenerico
		implements Serializable, GestionArbolDeterminaciones {
	@Logger
	private Log log;
	...
	public DefaultTreeModel getModel() {
		...
		if (model == null
			...
			// Como es un arbol nuevo de determinaciones nuevo, pongo la determinacion de trabajo a cero
	...
	public void nodeSelected(ActionEvent event) {
		...
		if (!node.isRoot()) {
			...
			// Meto la determinacion en GestionDeterminaciones
			//variablesSesionUsuario.setIdDeterminacionTrabajo(idDeterminacionBD);
			gestionDeterminaciones.setIdDeterminacion(idDeterminacionBD);
			
			

			log
					.info("[nodeSelected] Se ha guardado la variable setIdDeterminacionTrabajo "
							+ idDeterminacionBD + " en variablesSesionUsuario");
		...
		{
			// Como es el nodo raiz, pongo la determinacion de trabajo a cero
			gestionDeterminaciones.setIdDeterminacion(0);
		}
	...
	{
		if (nodo!=-1){
			log.debug("[expandirNodo] Expandir el nodo: "+nodo);
	...
	public void actualizoArbolDespuesDeGuardar() {
		...
		log.debug("[actualizoArbolDespuesDeGuardar] nodoId=" + nodoSeleccionado +" det a expandir="+gestionDeterminaciones.getIdDeterminacion());
		...
		try {
			...
			if (nodoSeleccionado!=0)
			{
				...
				// Cuando termino de actualizar, centro sobre la determinacion que hemos creado
			...
			e.printStackTrace();
			log.error("[actualizoArbolDespuesDeGuardar] No se podido actualizar el arbol de determinaciones");
	...
	public void cargarYExpandirDeterminacion(int idDeterminacion) {
		
		if(idDeterminacion != 0) {
			getModel();
			expandirNodo(idDeterminacion);
			gestionDeterminaciones.setIdDeterminacion(idDeterminacion);
		}		
	}
