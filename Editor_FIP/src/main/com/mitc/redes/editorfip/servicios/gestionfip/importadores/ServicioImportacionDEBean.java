...
@Stateful
@Name("servicioImportacionDEBean")
public class ServicioImportacionDEBean implements ServicioImportacionDE{
  ...
  //servicios
	...
	@In (create = true, required = false)
	ImportadorDEExcel importadorDEExcel;
	...
	public ServicioImportacionDEBean() {
//		super();
			...
			if (!servicioBasicoDeterminaciones.tieneDeterminacionTramite(getIdTramiteTrabajo()))
			{
						...
						&& validarOrden(sheet, listadoErrores))
				{
					...
					for (indiceFila=2; !fincu & indiceFila<sheet.getRows(); indiceFila++)
					{
						...
						{
							log.info("caracter="+registroCU[caracter].getContents());
							...
							if (idCaracter!=-1)
							{
								...
								if (registroCU[1].getContents()!=null && !registroCU[1].getContents().equalsIgnoreCase(""))
								{
									...
									if (registroCU[equivale].getContents()!=null && !registroCU[equivale].getContents().equals("")){
										dt= servicioBasicoDeterminaciones.obtenerEquivalente(getEquivalenteFormateado(registroCU[equivale].getContents()),getIdTramiteBaseTrabajo());
										if (dt!=null)
				...
				else
				{
					...
					List<String> erroresServicio = importadorDEExcel.persistirDEdeExcel(getIdTramiteTrabajo(), listaDEenExcel);
	...
	private DEExcelDTO cargarDEDTOValoresBase (Cell[] registroCU, int campo, int idCaracter, int fila)
	{
		...
		determinacion.setNombreDeterminacion(registroCU[campo+1].getContents());
		// Cargamos el orden
		...
		for (indiceFila=2; !finTotal & indiceFila<sheet.getRows(); indiceFila++)
		{
			...
			else
			{
				...
				for (indice=indiceFila+1; !fin & indice<sheet.getRows(); indice++)
				{
					...
					{
						listadoErrores.add("El orden se encuentra repetido en las filas: " +indiceFila + " y " + indice);
						correcto=false;
					}
				}
			}
