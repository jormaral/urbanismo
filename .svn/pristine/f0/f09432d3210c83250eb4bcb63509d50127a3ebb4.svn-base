...
@Stateful
@Name("gestionArbolEntidadesPadre")
public class GestionArbolEntidadesPadreBean extends GestionArbolGenerico
		implements Serializable, GestionArbolEntidadesPadre {
	@Logger
	private Log log;
	...
	public void cargarArbolInicial() {
		...
		log.debug("[cargarArbolInicial] Inicio");
		...
		if (idTramiteTrabajo != 0) {
			...
			DefaultMutableTreeNode rootNode = addNode(null,
					messages.get("entidad_no_tramite"),
					new ParIdentificadorTexto(0, "", ""));
			model = new DefaultTreeModel(rootNode);
	...
	public void guardarEntidadBase (ActionEvent event)
	{
		if (idEntidadBaseSeleccionada!=0)
		{
			...
			gestionEntidades.getEntidad().setEntidadByIdentidadbase(entBase);
