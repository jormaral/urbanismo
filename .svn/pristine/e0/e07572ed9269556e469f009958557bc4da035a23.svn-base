 ...
 *
 */
@Stateless(name="PlanEsOperadoPor")
public class PlanOperadoPorHelper implements IJsonHelper {
	...
	public Map<String, Object> marshal(Object id, String idioma) {
		...
		for (Operacionplan operacionPlan : plan.getOperacionplansForIdplanoperado()) {
			//ESTE PLAN ES OPERADO POR...
			operador=operacionPlan.getPlanByIdplanoperador();
			
			//OBTENGO LOS NOMBRES DE LOS PLANES ENCONTRADOS
            String nombreOperador="-";
            if(operador!=null)
            {
                nombreOperador=operador.getNombre();
            }
            
            linea = new HashMap<String, Object>();
            linea.put("iden", plan.getIden());
            linea.put("planOperador", nombreOperador);
            itop = servicioDiccionario.get(Instrumentotipooperacionplan.class, operacionPlan.getIdinstrumentotipooperacion());
