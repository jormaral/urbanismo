...
var MooTreeControl_Capas = new Class({
    ...
    initialize: function(config, options) {
        ...
        this.theme = config.theme || 'javascripts/mooTree2/mootree.gif';

        this.loader = config.loader || {
            icon:'javascripts/mooTree2/mootree_loader.gif',
            text:'Loading...',
            color:'#a0a0a0'
        };
...
var MooTreeNode_Capas = new Class({
    ...
    initialize: function(options) {
        ...
        if(options.data && options.data.visibility=='true')
        {
            ...
            {
                var estado=visor.generarNuevaCapaSegunNodeArbol(options.text);
        ...
        if(options && options.data && options.data.type=='WMS')
        {
                ...
                if (JV==null)
                {
                    ...
                    else
                    {
                        ...
                        if(url)
                        {
                            ...
                            var capa=visor.getLayerLegendDeCapaSiLoTiene(this.options.text);
        ...
        if(this.div.checkbox)
        {
            this.div.checkbox.onclick =this.div.checkbox.ondblclick = function(checkbox) {
                ...
                {
                    var layer=visor.map.getLayer(this._node.text)
                    if(layer!=null && layer!=undefined)
                    {
                        visor.map.removeLayer(layer);
                    }
                    this._node.div.main.destroy();
                    if(visor.layerVector){
                        //visor.layerVector.setZIndex((layerCreada.getZIndex()).toInt()+5);
                        visor.layerVector.display(false);
                        visor.layerVector.display(true);
                    }
                }
                else
                {
                    //this._node.control.select(this._node);
                    var estado=visor.generarNuevaCapaSegunNodeArbol(this._node.text);
                    visor.actualizarZIndexMarkers();
                    if(estado)
                    {
                        this._node.div.checkbox.setProperty('class','mooTree_checkON');
                        var layerCreada=visor.map.getLayer(this._node.text)
                        if (layerCreada){
                            layerCreada.setOpacity(($('knobElement_'+this._node.text).getStyle('left').toInt())/100);

                            //                        if(this._node.data.exportPdf!=undefined && this._node.data.exportPdf=='true')
                            //                        {
                            //                            this._node.div.icon.getFirst().setStyle ('background-position', '-254px 0px');
                            //                            this._node.div.icon.getFirst().setStyle ('cursor', 'pointer');
                            //                        }
                            //                        else
                            //                        {
                ...
                if (el!=null){
                    var s=new Slider(el,el.getElement('div[class=knobCapa]'), {
                        ...
                        onChange: function(value){
                            ...
                            bug.log("El valor del slider es: "+value,'MOOTREE_CAPAS');
                            var layer=visor.map.getLayer(jsIO.cargarTextoSegunIdioma(this.text));
                            if(layer)
                            {
	...
	*/
    ...
    getImg: function(name) {
        ...
        if (name != '') {
            ...
            {
                html = '<div alt="' + i + '" class="mooTree_imgLeyenda" tip="' + jsIO.cargarTextoSegunIdioma("txt_leyenda") + ' ' + this.text + '" ';
	...
	*/
    ...
    _checkBaseLayerGoogle:function(nodo){
        if(visor.baseLayerGoogle==true){
            return true;
        }
        if(visor.baseLayerGoogle==false && nodo.getAttribute('type')=='folder' && nodo.getAttribute('text')=='GOOGLE'){
            return false;
        }
        if(visor.baseLayerGoogle==false && nodo.getAttribute('type')=='GOOGLE'){
            return false;
        }
        return true;
    },
