...
var MooTreeControl = new Class({
    ...
    initialize: function(config, options) {
        ...
        this.progressive_chunk = options.progressive_chunk;
...
var MooTreeNode = new Class({
    ...
    initialize: function(options) {
        ...
        this.progressive_chunk = options.progressive_chunk || 5; //chunk size of progressive load
	...
	*/
    ...
    _import_progressive: function(inst, e,index){
        ...
        var chunklength = this.progressive_chunk || 5;
        for (var i=index, len = n.length; i<len; i++) if (n[i].tagName == 'node') {
            ...
            node.progressive_chunk = this.progressive_chunk;
            if (node.data.load) {
                node.open = false; // can't have a dynamically loading node that's already open!
                node.insert(this.control.loader);
                node.onExpand = function(state) {
            ...
            if (i > 0 && i+1 < len && i % chunklength == 0 ){
                setTimeout(function(){inst._import_progressive(inst, e, i+1);},5);
                break;
            }
