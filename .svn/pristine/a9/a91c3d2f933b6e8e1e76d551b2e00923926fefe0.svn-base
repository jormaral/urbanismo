  ...
  Implements: Options,
	...
	initialize: function(obj, options) {
		...
		this.calendar = new Element('div', { 
			'styles': { left: '-1000px', opacity: 0, position: 'absolute', top: '-1000px', zIndex: 99999999 }
		}).addClass(this.classes.calendar).injectInside(document.body);

		// iex 6 needs a transparent iframe underneath the calendar in order to not allow select elements to render through
		if (window.ie6) {
			this.iframe = new Element('iframe', { 
				'styles': { left: '-1000px', position: 'absolute', top: '-1000px', zIndex: 99999999 }
		...
		for (var i in obj) {
			var cal = {
                id:'buttonCalendar',
				button: new Element('button', { 'type': 'button', 'id': 'buttonCalendar' }),
				el: $(i),
				els: [],
				id: id++,
				month: d.getMonth(),
	...
	clicked: function(td, day, cal) {
		...
		buquedaModificadaVisorPlanes = true;
		if (cal.el && $(cal.el.id+'Visible') != undefined){  //Si tiene un visible sera porque es editable
			$(cal.el.id+'Visible').value = cal.el.value;			
		}
		if ($(cal.el.id+'Visible').get('data-meiomask') != null){ //Si tiene mascara hay que hacer el link otra vez
			$(cal.el.id+'Visible').meiomask($(cal.el.id+'Visible').get('data-meiomask'));
		}
	},
