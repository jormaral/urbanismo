 ...
 *
 */
@Stateless(name="TipoTramite")
public class TipoTramiteHelper implements IJsonHelper {
	...
	public Map<String, Object> marshal(Object id, String idioma) {
		...
		Tipotramite[] tramites = servicioDiccionario.get(Tipotramite.class);
		
		Map<String, Object> bj; 

		for (int i = 0; i < tramites.length;i++) {
			bj = new HashMap<String, Object>();
			bj.put("iden", tramites[i].getIden());
			bj.put("nombre",  servicioDiccionario.getTraduccion(Tipotramite.class, tramites[i].getIden(), idioma));
			listaTipos.add(bj);
		}
