 ...
 *
 */
@Stateless(name="GET_INSTRUMENTOSPLAN")
public class GetInstrumentosPlanes implements IAccion {
	...
	public void ejecutar(HttpServletRequest request,
		...
		if (idPlan != null &&
			...
			Instrumentoplan[] instrumentos = servicioPlaneamiento.getInstrumentosPlan();
    		...
    		try {
                ...
                int num;
                for (Instrumentoplan ip : instrumentos) {
                	
                	num = servicioPlaneamiento.getNumeroPlanesHijos(Integer.parseInt(idPlan),ip.getIden(), ModalidadPlanes.valueOf(tipo));
                	
                    if (num > 0) {
                    	xtw.writeStartElement("node");
                        xtw.writeAttribute("text", servicioDiccionario.getTraduccion(Instrumentoplan.class, ip.getIden(), idioma)+ " (" + num + ")");
                        xtw.writeAttribute("idNode",String.valueOf(ip.getIden()));
