...
/**
 * 
 * Marca la determinación operada como suspendida.
 * 
 * @author Arnaiz Consultores
 *
 */
@Stateless(name = "refundido.operacion.determinaciones.suspension")
public class Suspender implements EjecutorLocal {
	...
	public void ejecutar(int idOperada, int idOperadora,
		...
		contexto.logTraducido(LOG.INFO, "refundido.operacion.determinacion.suspension.mensaje",
		...
		Determinacion suspendida = referencias.getReferencia((int)contexto.getParametro(ContextoRefundido.ID_PROCESO), Determinacion.class, idOperada);
		...
		if (operadora != null && !operadora.isBsuspendida()) {
			if (suspendida != null) {
				suspendida.setBsuspendida(true);
			} else {
