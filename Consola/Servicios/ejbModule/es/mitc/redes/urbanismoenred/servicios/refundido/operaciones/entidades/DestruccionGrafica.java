 ...
 *
 */
@Stateless( name = "refundido.operacion.entidades.destruccionGrafica")
public class DestruccionGrafica implements EjecutorLocal {
	...
	public void ejecutar(int idOperada, int idOperadora,
		...
		contexto.logTraducido(LOG.INFO, "refundido.operacion.entidad.destruccionGrafica.mensaje", 
		...
		if (operada != null && operadora != null) {
			if (!operadora.isBsuspendida()) {
				for (Entidadpol poligono : operada.getEntidadpols()) {
					em.remove(poligono);
				}
				for (Entidadlin linea : operada.getEntidadlins()) {
					em.remove(linea);
				}
				for (Entidadpnt punto : operada.getEntidadpnts()) {
					em.remove(punto);
				}
