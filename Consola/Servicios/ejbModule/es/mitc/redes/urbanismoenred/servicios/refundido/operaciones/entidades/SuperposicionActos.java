 ...
 *
 */
@Stateless( name = "refundido.operacion.entidades.superposicionActos")
public class SuperposicionActos extends CopiarEntidadDeterminacion implements EjecutorLocal {
	
	private static final List<Integer> CARACTERES = Arrays.asList(ClsDatos.ID_CARACTER_ACTODEEJECUCION, ClsDatos.ID_CARACTER_GRUPODEACTOS);
	...
	public void ejecutar(int idOperada, int idOperadora,
		...
		contexto.logTraducido(LOG.INFO, "refundido.operacion.entidad.superposicionActos.mensaje", 
		...
		if (operada != null && operadora != null) {
			if (!operadora.isBsuspendida()) {
				contexto.putParametro(ES_SUPERPOSICION, true);
