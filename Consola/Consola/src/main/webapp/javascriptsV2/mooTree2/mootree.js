...
Properties:
	...
	searched - returns the currently searched node <MooTreeNode> object
	searchPath - returns the currently search path
...
var MooTreeControl = new Class({
	...
	initialize: function(config, options) {
		...
		this.theme = config.theme || 'javascriptsV2/mooTree2/mootree.gif';
		
		this.loader = config.loader || {
            icon:'javascriptsV2/mooTree2/mootree_loader.gif',
            text:'Cargando...',
            color:'#a0a0a0'
        };
		
		this.selected = null;                 // set the currently selected node to nothing
		this.searchPath = null;				  // set de search path to nothing
		this.searched = null;				  // set de searched node to nothing
		...
		this.treeType = config.treeType;	  // type of grid (planes, determinaciones, entidades, usos, actos,...)
		this.iconBar = config.iconBar;		  // gif or gifs to get the icons nodes
	...
	*/
	...
	find: function(search){ 
		...
		return this._find(search);
	},
	
	findNext: function(){ 
		//Busco el siguente elemento sobre el searchPath actual inicializando el nodo al selected
		this.searched = null;
		return this._find(this.searchPath);
	},
		
	_find: function(search) {
		if (this.searched ==null){
			if (!this.selected){	//Si no hay ninguno seleccionado selecciono el primero		
				this.selected = this.root;			
			}
			this.searched = this.selected;
		}				
		
		//Si no tiene datos los cargo
		this.searched._expandSync();
		
		var nodeToSearch = this.searched;
		var subNodes = nodeToSearch.nodes;
		var dataToSearch;
		var i=0;
		var j=0;
		var reSelected = -1; //-1 no se ha seleccionado nada, 0 es una carpeta, 1 es un nodo completo
		var dev = 0; //indica si la ruta se ha cerrado (-1) o sigue abierta
		for (i=0; i<search.length; i++) { //selecciono un objeto de busqueda
			dataToSearch = search[i];		
			if (dataToSearch.id != -1) //Si la ruta esta abierta
			{
				for (j=0; j<subNodes.length; j++) { //busco entre los nodos de este nivel	
					if (!subNodes[j].data.tipo && dataToSearch.tipo == 'instrumentoplan' && subNodes[j].data.idNode == dataToSearch.id){ //Se trata de una carpeta de instrumento plan
						//Si el nodo seleccionado coincide con el buscado, lo selecciono y expando
						this.searched = subNodes[j];
						...
						reSelected = 1;
						break;
					} else if (subNodes[j].data.tipo == 'infoplan' && this.treeType == 'planes'){ //Se trata de una carpeta de planes
						//Si el nodo seleccionado coincide con el buscado, lo selecciono y expando
						this.searched = subNodes[j];
						...
						reSelected = 0;
						break;
					} else if (subNodes[j].data.idNode == dataToSearch.id && subNodes[j].data.tipo == dataToSearch.tipo){
						//Si el nodo seleccionado coincide con el buscado, lo selecciono y expando
						this.searched = subNodes[j];
				...
				//Si ya he seleccionado uno hay que bajar un nivel
				if (reSelected > -1){						
					if (reSelected == 0){ //ES UNA CARPETA, NO SE AUMENTA EL dataToSearch
						var resultSearch = this._find(search);
						if (resultSearch == -1 && !this._TieneHijosAbiertos(dataToSearch.siguiente)){ //si la ruta esta cerrada y no hay hijos con ruta abierta se cierra el propio nodo y la ruta
							dataToSearch.id = -1;
							dev = -1;
						}
					} else if (reSelected == 1){ //Se han encontrado datos, se continua si NO es resultado y hay algo
						if (dataToSearch.resultado==false && dataToSearch.siguiente && dataToSearch.siguiente.length > 0){ //El siguiente tiene datos	
							var resultSearch = this._find(dataToSearch.siguiente);						
							if (resultSearch == -1 && !this._TieneHijosAbiertos(dataToSearch.siguiente)){ //si la ruta esta cerrada y no hay hijos con ruta abierta se cierra el propio nodo y la ruta
								dataToSearch.id = -1;
								dev = -1;
							}
						} else {
							//Si no hay hijos cierro el nodo y la ruta, pero si hay, solo cierro la ruta
							if (dataToSearch.siguiente && dataToSearch.siguiente.length > 0){
								dataToSearch.resultado=false;								
								dev = -1;
							} else {
								dataToSearch.resultado=false;
								dataToSearch.id = -1;
							...
							//Scroll al nodo seleccionado
							//var scroll = this.searched.div.main.getScrollSize();
							this.root.div.main.parentNode.parentNode.parentNode.scrollTo(0,this.searched.div.text.offsetTop); //scroll.x
							//this.root.div.main.parentNode.parentNode.parentNode.scrollTop = this.root.div.main.parentNode.parentNode.parentNode.scrollHeight;
							
							$$('.mooTree_selected_search').each(function (elem) {
								elem.removeClass('mooTree_selected_search');
						    });
							this.searched.div.text.addClass("mooTree_selected_search");							
						}
					}					
					break;
				}
		...
		if (reSelected == -1) { //No se han encontrado mas datos, cierro nodo y ruta
			dataToSearch.id = -1;
			dev = -2;
		}
		
		return dev; //Devuelvo el estado de la ruta
	},
	
	
	_TieneHijosAbiertos: function(hijos){
		var dev = false; //supongo que no tiene
...
var MooTreeNode = new Class({
	...
	initialize: function(options) {
		...
		//Si no se trata del nodo de cargando
		if (this.text != "Cargando..."){
			this.icon = this._getIconNumber(this.control.treeType, this.data, this.control.iconBar) || options.icon;
		} else {
	...
	*/
	...
	/*
	Property: _getIconNumber
		Get the gif file and number into the gif array about the type and node data
	*/
	
	_getIconNumber: function(type, data, iconfile) {
		
		var iconNumber = undefined;
		
		if (type && data && iconfile){
		    ...
		    case 'entidades':
		    	iconfile = iconfile[0];
		    	if (data.codigoGrupoEntidad){
		    		switch(data.codigoGrupoEntidad){
		    		case '7000000002':
		    			iconNumber = '1';
		    			break;
		    		case '7000000098':
			...
			if (iconNumber){
				if (iconNumber > 0) {
					iconNumber = iconfile + '#' + iconNumber;
				} else {
					iconNumber = iconfile;
				}				
			}
	...
	*/
	...
	/*
	Property: expand
		Expand de current node and de childrens
	
	 */
	expand: function(){
		this.toggle(false, true);
		this.nodes.forEach( function(node) { node.toggle(false, true); });
		/*
		if (this.data.tipo || this.control.root.text == this.text){ //Expando a el y el siguiente
			this.toggle(false, true);
			this.nodes.forEach( function(node) { node.toggle(false, true); });
		} else { //Expando solo a el
			this.control.selected.toggle(false, true);
		}*/
	},
	
	/*
	  Property: _expandSync
	  	Solo para uso interno, se hace uso de esta propiedad durante la busqueda de elementos
	 */
	_expandSync: function(){
		if (this.nodes.length == 1 && this.nodes[0].text == 'Cargando...'){
			this.open = true;
			this.loadSinchronously(this.data.load);
			this.control.onExpand(this, this.open);
		} else {
			this.open = true;
		}		
	...
	*/
	...
	loadSinchronously: function(url,vars){
	...
	/*
	Property: loadData
		Load an XML structure passed by parameter into a node of this tree.
	
	Parameters:
		xmlData - XML structure to load
	*/
	
	loadData: function(xmlData) {
		
		if (this.loading || !xmlData) return; // if this node is already loading, return
		this.loading = true;      // flag this node as loading
			
		this.clear();
		
		this._loaded("",xmlData);
	},
