 ...
 *
 */
@Stateless(name="GET_NODOS_DETS_HIJOS_POR_TIPO")
public class GetDeterminacionesHijasTipo implements IAccion {
	...
	public void ejecutar(HttpServletRequest request,
		...
		if (idEnt != null && tipo != null) {
	    	...
	    	if (tipo.equalsIgnoreCase("regimendirecto")) {
	    		determinaciones = servicioPlaneamiento.getDeterminacionesHijasPorRegimenDirecto(Integer.parseInt(idEnt));
	        } else if (tipo.equalsIgnoreCase("acto")) {
	        	determinaciones = servicioPlaneamiento.getDeterminacionesHijasPorActo(Integer.parseInt(idEnt));
	        } else if (tipo.equalsIgnoreCase("uso")) {
	        	determinaciones = servicioPlaneamiento.getDeterminacionesHijasPorUso(Integer.parseInt(idEnt));
	        } else {
			...
			try {
	            ...
	            for (Determinacion det : determinaciones) {
	            	...
	            	xtw.writeAttribute("text", det.getApartado() +" "+ det.getNombre());
	            	...
	            	xtw.writeAttribute("load", "GestionConsola?wsName=GET_NODOS_DETS_HIJOS_POR_TIPO&idDet=" 
	            			+ det.getIden() + "&tipo=" + URLEncoder.encode(tipo,"UTF-8"));
	            	xtw.writeEndElement();
	            }
	            xtw.writeEndElement();
