 ...
 *
 */
@Stateless(name="BORRAR_ENT_PLAN_BASE")
public class BorrarEntidad implements IAccion {
	private static final Logger log = Logger.getLogger(BorrarEntidad.class);
	...
	public void ejecutar(HttpServletRequest request,
		...
		data.put("affectedId", idEnt);
		if (idEnt != null) {
			try {
				Entidad entidad = servicioPlaneamiento.get(Entidad.class, Integer.parseInt(idEnt));
				if (entidad != null) {
					int ambito = entidad.getTramite().getPlan().getIdambito();
					servicioPlaneamiento.borrarEntidad(Integer.parseInt(idEnt));
					...
					servicioDiario.nuevoRegistroDiario(((Usuario)request.getSession().getAttribute("usuario")).getNombre()
							...
							, "Borrado de la entidad " + idEnt);
					data.put("error", false);
				} else {
					data.put("error", true);
					data.put("result", "No existe la entidad en base de datos");
				}
