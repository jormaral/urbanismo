...
var MooFlow = new Class({
	...
	initialize: function(element, options){
		...
		this.foc = 150;
		this.factor = (visor.factorLeyenda).toInt();
	...
	setScreen: function(){
		if(this.isFull = !this.isFull){
			...
			this.MooFlow.setStyles({'position':'absolute','z-index':'9999999','top':'0','left':'0','width':window.getSize().x,'height':window.getSize().y});
                        this.MooFlow.factor=60;
			...
			this.MooFlow.wraps(this.holder);
                        this.MooFlow.factor=100;
			window.removeEvent('resize', this._initResize);
			delete this.holder, this._initResize;
			this.MooFlow.setStyles({'position':'relative','z-index':'','top':'0','left':'0','width':'100%','height':this.MooFlow.retrieve('height')});
